...
class DemoAI:
    ...
    def update(self, dt, player, miner, enemies, game):
        ...
        if not player or not miner:
            return 0, 0, False, False

        # Calculate direction to miner
        dx = miner.x - player.x
        dy = miner.y - player.y
        distance_to_miner = math.sqrt(dx*dx + dy*dy)

        # Horizontal movement toward miner
        move_x = 0
        if abs(dx) > 10:  # Dead zone
            move_x = 1 if dx > 0 else -1

        # Vertical movement - fly if miner is above or if falling
        move_y = 0
        if dy < -20 or player.vel_y > 50:  # Fly if miner is above or falling fast
            move_y = -1
        ...
        if self.shoot_timer <= 0:
            for enemy in enemies:
                ...
                enemy_dx = enemy.x - player.x
                enemy_dy = enemy.y - player.y
                enemy_distance = math.sqrt(enemy_dx*enemy_dx + enemy_dy*enemy_dy)

                if enemy_distance < 150:  # Enemy nearby
                    # Check if facing the right direction
                    if (player.facing_right and enemy_dx > 0) or (not player.facing_right and enemy_dx < 0):
                        if abs(enemy_dy) < 50:  # Roughly same height
                            shoot = True
                            self.shoot_timer = self.shoot_cooldown
                            break

        # Use dynamite if close to blocks and have bombs
        use_dynamite = False
        if game.bombs > 0 and distance_to_miner > 100:
            # Check for blocks nearby
            tile_x = int(player.x / TILE_SIZE)
            tile_y = int(player.y / TILE_SIZE)
            level_map = MAPS[game.level]
            ...
            for offset_y in range(-1, 2):
                for offset_x in range(-1, 2):
                    ...
                    if 0 <= check_y < len(level_map) and 0 <= check_x < len(level_map[0]):
                        if level_map[check_y][check_x] == 'B':
                            use_dynamite = True
                            break

        return move_x, move_y, shoot, use_dynamite
...
class Player:
    ...
    def update(self, dt, keys, joy_axis_x, joy_axis_y, level_map, game, ai_input=None):
        """Update player physics and movement

        ai_input: tuple (move_x, move_y, shoot, dynamite) for demo mode
        """
        ...
        if ai_input:
            hover_input = ai_input[1] < 0
        else:
            hover_input = keys[pygame.K_UP] or joy_axis_y < -DEAD_ZONE
...
class Game:
    def __init__(self):
        ...
        # Demo mode
        self.demo_ai = DemoAI()
        self.demo_timer = DEMO_SWITCH_TIME
        self.demo_level_index = 0
        self.background_image = None
        self.screenshot_taken = False
        self.demo_play_time = 0  # Track how long demo has been playing
    ...
    def init(self):
        ...
        # If no background exists, start demo quickly to generate one
        if not self.background_image:
            self.demo_timer = 1.0  # Start demo after 1 second
            print("No background image found, will generate from demo...")

    def load_background(self):
        """Load or create background image for splash screen"""
        try:
            if os.path.exists(SCREENSHOT_FILE):
                self.background_image = pygame.image.load(SCREENSHOT_FILE).convert()
                ...
                print(f"Loaded background image: {SCREENSHOT_FILE}")
            else:
                # Create procedural background if no screenshot exists
    ...
    def create_procedural_background(self):
        """Create a procedural background image representing the game"""
        print("Creating procedural background image...")
        try:
            # Create a surface
            bg = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT))
            bg.fill(COLOR_BLACK)

            # Render a sample level (level 0)
            level_map = MAPS[0]
            for row_index, row in enumerate(level_map):
                for col_index, tile in enumerate(row):
                    ...
                    if tile == "#":
                        bg.blit(self.tiles['wall'], (x, y))
                    elif tile == ".":
                        bg.blit(self.tiles['floor'], (x, y))
                    elif tile == "B":
                        bg.blit(self.tiles['block'], (x, y))
                    else:
                        bg.blit(self.tiles['blank'], (x, y))

            # Draw a sample player
            player_x = 64
            player_y = 64
            pygame.draw.rect(bg, COLOR_BLUE, (player_x + 8, player_y + 12, 16, 12))
            pygame.draw.line(bg, COLOR_WHITE, (player_x, player_y + 8), (player_x + 32, player_y + 8), 2)
            pygame.draw.circle(bg, (100, 150, 255), (player_x + 16, player_y + 16), 6)

            # Draw a sample enemy
            enemy_x = 200
            enemy_y = 150
            pygame.draw.circle(bg, COLOR_RED, (enemy_x + 16, enemy_y + 16), 12)
            pygame.draw.circle(bg, COLOR_RED, (enemy_x + 8, enemy_y + 16), 6)
            pygame.draw.circle(bg, COLOR_RED, (enemy_x + 24, enemy_y + 16), 6)

            # Draw a sample miner
            miner_x = 400
            miner_y = 320
            pygame.draw.circle(bg, COLOR_GREEN, (miner_x + 16, miner_y + 10), 8)
            pygame.draw.rect(bg, COLOR_GREEN, (miner_x + 12, miner_y + 18, 8, 12))

            # Save it
            pygame.image.save(bg, SCREENSHOT_FILE)
            print(f"Created and saved procedural background: {SCREENSHOT_FILE}")

            self.background_image = bg
        except Exception as e:
            print(f"Error creating procedural background: {e}")
            self.background_image = None
    ...
    def start_demo(self):
        ...
        self.demo_level_index = 0
        self.level = DEMO_LEVELS[self.demo_level_index]
        self.score = 0
        self.lives = 3
        self.bombs = 5
        self.last_life_score = 0
        self.demo_play_time = 0  # Reset play time
        self.start_level()
    ...
    def rescue_miner(self):
        ...
        if not self.miner.rescued:
            ...
            # Set level complete timer (shorter for demo)
            if self.state == STATE_DEMO:
            ...
            # Restore demo state if we were in demo
            if prev_state == STATE_DEMO:
                self.state = STATE_LEVEL_COMPLETE  # Will be handled in update_level_complete
    ...
    def update_level_complete(self, dt):
        ...
        if self.level_complete_timer <= 0:
            if self.state == STATE_DEMO:
                # In demo mode, advance to next demo level
                self.demo_level_index = (self.demo_level_index + 1) % len(DEMO_LEVELS)
                self.level = DEMO_LEVELS[self.demo_level_index]
                self.state = STATE_DEMO
                self.start_level()

                # Take screenshot on first level completion if not taken yet
                if not self.screenshot_taken and self.demo_level_index == 1:
    ...
    def update_demo(self, dt):
        """Update demo mode"""
        # Track demo playtime
        self.demo_play_time += dt

        # Get AI input
        ai_move_x, ai_move_y, ai_shoot, ai_dynamite = self.demo_ai.update(
            dt, self.player, self.miner, self.enemies, self)

        # Create simulated input for player
        ai_input = (ai_move_x, ai_move_y, ai_shoot, ai_dynamite)

        # Handle AI shooting
        if ai_shoot and self.shoot_cooldown <= 0:
            self.shoot()

        # Handle AI dynamite
        if ai_dynamite and self.bombs > 0:
            self.place_dynamite()

        # Update player with AI input
        if self.player:
            self.player.update(dt, self.keys, 0, 0, MAPS[self.level], self, ai_input)
        ...
        # Handle demo death - restart demo
        if self.energy <= 0 or self.lives <= 0:
            self.start_demo()

        # Take screenshot midway through first level if not taken
        if not self.screenshot_taken and self.demo_level_index == 0:
            if self.player and len(self.enemies) > 0:
                # Take screenshot when there's action (player, enemies visible)
                if any(enemy.active for enemy in self.enemies):
                    # Only take after some gameplay
                    if self.score > 0 or len(self.bullets) > 0:
                        # Screenshot will be taken next render
                        pass
    ...
    def render_splash(self):
        ...
        if self.background_image:
            ...
            bg_copy.set_alpha(100)  # Semi-transparent
    ...
    def loop(self):
        ...
        while running:
                ...
                if event.type == pygame.QUIT:
                    ...
                    if self.state == STATE_SPLASH:
                        if event.key == pygame.K_SPACE:
                            ...
                            self.bombs = 5
                            self.last_life_score = 0
                            self.demo_timer = DEMO_SWITCH_TIME  # Reset demo timer
                            self.start_level()
                        elif event.key == pygame.K_ESCAPE:
                            running = False

                    elif self.state == STATE_DEMO:
                        # Any key press returns to splash
                        self.state = STATE_SPLASH
                        self.demo_timer = DEMO_SWITCH_TIME
                    ...
                    if self.state == STATE_SPLASH:
                        ...
                        # Any button press returns to splash
                        self.state = STATE_SPLASH
                        self.demo_timer = DEMO_SWITCH_TIME
            ...
            if self.state == STATE_SPLASH:
                ...
                if self.miner and self.miner.rescued:
                    pass  # Will be handled by rescue_miner
            ...
            if self.state == STATE_SPLASH:
                ...
                # Take screenshot for background if conditions are met
                if not self.screenshot_taken and self.demo_level_index == 0:
                    # Take screenshot after 3 seconds of gameplay with some action
                    if self.demo_play_time > 3.0 and self.player:
                        if len(self.enemies) > 0 or self.score >= 0:  # Always true after 3 seconds
                            print(f"Taking screenshot at {self.demo_play_time:.1f}s, score={self.score}")
                            # Capture screenshot BEFORE adding overlay
                            self.capture_screenshot()
                            # Reload it for immediate use
                            self.load_background()
                ...
                demo_text = self.small_font.render("DEMO MODE - Press any key to play", True, COLOR_YELLOW)
                demo_rect = demo_text.get_rect(center=(SCREEN_WIDTH // 2, 20))
